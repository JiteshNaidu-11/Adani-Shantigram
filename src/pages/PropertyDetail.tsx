...
export default function PropertyDetail() {
  ...
  const areaDisplay = [
    project.saleableArea ? `Saleable: ${project.saleableArea}` : "",
    project.usableArea ? `Usable: ${project.usableArea}` : "",
  ]
    .filter(Boolean)
    .join(" | ");

  const infoCards = [
    { label: "Configuration", value: project.configuration ?? project.type, icon: Sofa },
    { label: "Possession", value: project.possessionDate ?? "TBA", icon: Clock },
    { label: "Area", value: areaDisplay || project.area, icon: Ruler },
    { label: "Floors", value: project.floors ?? "TBA", icon: Layers3 },
    { label: "Units", value: project.totalUnits ?? "TBA", icon: Building2 },
    { label: "Price Range", value: project.priceRange ?? priceDisplay, icon: Wallet },
    { label: "Parking", value: project.carParking ?? "TBA", icon: Building2 },
  ];
      ...
      <div className="container-custom mx-auto px-4 sm:px-6 max-w-6xl">
        ...
        >
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            <div>
              <h1 className="font-display text-2xl sm:text-3xl text-foreground mb-1">{project.name}</h1>
              <p className="text-muted-foreground text-sm mb-1">{project.location ?? LOCATION_TEXT}</p>
              <p className="text-foreground font-medium text-sm">{priceDisplay}</p>
            </div>
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Button size="sm" variant="outline" className="rounded-full gap-1.5" onClick={openLead}>
        ...
        >
          {infoCards.map(({ label, value, icon: Icon }) => (
            <div key={label} className="flex items-start gap-3 p-3 rounded-xl border border-border bg-card">
              <Icon size={22} className="text-accent flex-shrink-0 mt-0.5" />
        ...
        <motion.div key={activeTab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
          {activeTab === "About" && (
            <div>
              <h2 className="font-display text-xl md:text-2xl text-foreground mb-4">
                {project.name}: {project.configuration ?? project.type}
              </h2>
              ...
              <div className="grid sm:grid-cols-2 gap-3 mb-4">
                <div className="rounded-xl border border-border bg-card p-4">
                  <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Booking Amount</p>
                  <p className="text-sm font-medium text-foreground">{project.bookingAmount ?? "TBA"}</p>
                </div>
                <div className="rounded-xl border border-border bg-card p-4">
                  <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Payment Terms</p>
                  <p className="text-sm font-medium text-foreground">{project.paymentTerms ?? "TBA"}</p>
                </div>
              </div>
              {longDesc.length > 280 && (
                <button
          ...
          {activeTab === "Amenities" && (
            <div>
              <h2 className="font-display text-xl md:text-2xl text-foreground mb-6">{project.name} - Amenities</h2>
              <div className="relative group/amenities">
                ...
                >
                  {amenities.map((amenity) => (
                    <div key={amenity.name} className="flex-shrink-0 snap-start w-32 text-center">
                      ...
                      <p className="text-xs font-medium text-foreground line-clamp-2">{amenity.name}</p>
                    </div>
                  ))}
                </div>
                <button
          ...
          {activeTab === "Master Layout" && (
            <div>
              <h2 className="font-display text-xl md:text-2xl text-foreground mb-4">Master Layout</h2>
              <p className="text-muted-foreground leading-relaxed mb-6">
                The Shantigram master plan includes residential zones, commercial spaces, The Belvedere Golf & Country
                Club, Adani International School, parks, and retail. {project.name} is part of this integrated layout.
              </p>
          ...
          {activeTab === "Typical Floor Plan" && (
            <div>
              <h2 className="font-display text-xl md:text-2xl text-foreground mb-4">Typical Floor Plan</h2>
              <p className="text-muted-foreground leading-relaxed mb-6">
                Floor plans for {project.name} show configuration, room sizes, and layout. Request the latest brochure
                for detailed typical floor plans and availability.
              </p>
          ...
          {activeTab === "Typical Unit Plans" && (
            <div>
              <h2 className="font-display text-xl md:text-2xl text-foreground mb-4">Typical Unit Plans</h2>
              <p className="text-muted-foreground leading-relaxed mb-6">
                Unit-wise plans for {project.configuration ?? project.type} at {project.name} include carpet area, room
                dimensions, and optional variants. Contact our sales team for unit plans and pricing.
              </p>
              <div className="rounded-xl border border-border bg-secondary/40 p-10 text-center max-w-xl mx-auto">
          ...
          {activeTab === "Location" && (
            <div>
              <h2 className="font-display text-xl md:text-2xl text-foreground mb-4">Location</h2>
